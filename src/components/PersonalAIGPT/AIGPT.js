...
import { MainPageButtonMenu } from './AIBlogContent/data';
import { 
    ...
    <>
      ...
      <div className='lg:w-[calc(100%-312px)] w-full lg:ml-[12px] h-[calc(100vh-24px)] overflow-y-auto '>
        ...
        <div className='m-[20px_0px] sm:m-[25px_8px_10px] flex flex-col text-center'>
          <p className='sm:text-[28px] text-[28px] font-[700] text-white mb-4'>Personal AI-GPT</p>
        ...
        <div className="flex flex-wrap justify-center sm:m-[0px] lg:py-[7rem] md:py-[7rem] py-[1rem]"> 
          {MainPageButtonMenu.map((item, index) => (
            <div 
              key={`action-${index}`}
              onClick={() => navigate(item.url)}
              className='lg:w-[183px] md:w-[183px] w-[260px] h-[171px] cursor-pointer rounded-[12px] border-[1px 0px 0px 0px] bg-[#3D4245] ml-[12px] mr-[12px] lg:mt-0 md:mt-0 mt-4'
            ...
            >
              ...
              <div className='pt-[8px] font-bold text-white text-center'>{item.name}</div>
              <div className='p-[8px] text-[12px] text-[#A8A8A8] text-center whitespace-normal'>{item.desc}</div> 
            </div> 
          ))}
        </div>
